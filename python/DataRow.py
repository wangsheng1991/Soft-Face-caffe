...
def cropFace(dataRows, destpath, destpathlist, IMAGE_SIZE=40, mirror=False):
    ''' Cropping image set using dlib from given valid data rows.
    ...
    output_file = open(destpathlist, 'w')
	...
	leftEyeIndex = int(round(dataRow.leftEye[0] + dataRow.leftEye[1] * 40))
        ...
        scipy.misc.imsave(destpath +'/'+ dataRowOrig.name, image)                
